...
const DESCRIPTION = "List files and directories at a given path. Returns a tree view of the directory structure."
    ...
    return {
      ...
      execute: (params: { path?: string; ignore?: string[] }, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          yield* assertExternalDirectoryEffect(ctx, searchPath, { kind: "directory" }, additionalDirs)
          ...
          const files = yield* rg.files({ cwd: searchPath, glob: ignoreGlobs, signal: ctx.abort }).pipe(
            Stream.take(LIMIT),
            Stream.runCollect,
