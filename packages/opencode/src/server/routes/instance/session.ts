...
export const SessionRoutes = lazy(() =>
  new Hono()
    ...
    .get(
      ...
      async (c) =>
        jsonRequest("SessionRoutes.directories", c, function* () {
          const sessionID = c.req.valid("param").sessionID
          const session = yield* Session.Service
          return yield* session.getDirectories(sessionID)
        }),
    )
    .delete(
      "/:sessionID/directory",
      ...
      async (c) =>
        jsonRequest("SessionRoutes.removeDirectory", c, function* () {
          const sessionID = c.req.valid("param").sessionID
          const { path } = c.req.valid("json")
          const session = yield* Session.Service
          yield* session.removeDirectory({ sessionID, path })
          return true
        }),
    ),
)
