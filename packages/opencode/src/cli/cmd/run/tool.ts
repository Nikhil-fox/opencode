...
function runCodeSearch(p: ToolProps): ToolInline {
...
function scrollCodeSearchStart(p: ToolProps): string {
  const query = text(dict(p.input).query)
  if (!query) {
...
function permCodeSearch(p: ToolPermissionProps): ToolPermissionInfo {
  const query = text(dict(p.input).query)
  return {
    icon: "◇",
    title: `Exa Code Search "${query}"`,
